...
router.get('/student-lookup', async (req, res) => {
  try {
    ...
    if (!roll || !cls) return res.status(400).json({ error: 'roll and class required' });
    const { rows } = await query(
      `SELECT id, name, roll_number, class, section, school_id
       FROM students
       WHERE roll_number = $1 AND class = $2
       LIMIT 1`,
      [roll, cls]
    );
    ...
    console.error('[sync/student-lookup]', err.message);
