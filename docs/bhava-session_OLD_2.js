// ── Bhāva Tech — Session Tracker + Game Nav v4 ───────────────────────────────
// Single unified file. Drop ONE tag before </body> in any game HTML:
//   <script src="../bhava-session.js"></script>
//
// IMPORTANT — set the game name on the <html> tag (not the title):
//   <html lang="en" data-bhava-game="Memory Match Puzzle">
//   This must exactly match the key in GAME_MAP inside main.js.
//   If data-bhava-game is absent, falls back to document.title (stripped).
//
// Call from your game's score logic when the game ends:
...
(function () {
  ...
  var REPORT_URL = '../bhava-student-report.html';   // path from docs/ subfolder

  // ── Resolve the canonical game name ──────────────────────────────────────
  // Priority: data-bhava-game attribute → cleaned document.title → 'Unknown Game'
  function resolveGameName() {
    // 1. Explicit attribute on <html> (recommended — must match GAME_MAP key exactly)
    var attr = document.documentElement.getAttribute('data-bhava-game');
    if (attr && attr.trim()) return attr.trim();

    // 2. Strip common suffixes from document.title
    var title = (document.title || '')
      .replace(/\s*[—–-]\s*Bh[aā]va\s*Tech\s*/gi, '')
      .replace(/\s*\|\s*Bh[aā]va\s*Tech\s*/gi, '')
      .replace(/Bh[aā]va\s*Tech\s*/gi, '')
      .replace(/\s*Game\s*$/i, '')
      .trim();
  ...
  function showLoginModal() {
    ...
    modal.innerHTML = '<div style="'
      ...
      + '<p style="color:#888;font-size:0.82rem;margin:0 0 6px">'
      + 'Playing: <strong>' + GAME_NAME + '</strong></p>'
      + '<p style="color:#aaa;font-size:0.78rem;margin:0 0 18px">'
  ...
  async function startSession() {
    ...
    try {
      currentSessionId = await window.bhava.startSession(
        ...
        GAME_NAME                              // ← uses resolved game name
      );
  ...
  function injectNavBar() {
    ...
    var gameTitle = GAME_NAME.slice(0, 32) || 'Bh\u0101va Game';
  ...
  function _openReportPage() {
    ...
    var base = (window.location.pathname.indexOf('/docs/') !== -1)
  ...
  window.BhavaSession = {
    ...
    logout:      clearStudent,
    setStudent:  function (student) { persistStudent(student); _syncToNav(); },

    openReport:     _openReportModal,
    openReportPage: _openReportPage,
    goHome:         _goHome,
    goBack:         _goBack,
    gameName:       function () { return GAME_NAME; },  // useful for debugging
  };
